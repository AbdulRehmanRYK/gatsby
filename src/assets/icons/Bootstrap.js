import React from 'react';

export default function BootstrapSvg(props) {
  return (
   <svg
      xmlns="http://www.w3.org/2000/svg"
      width="104"
      height="100"
      fill="none"
      viewBox="0 0 184 100"
    >
      <path fill="url(#pattern6)" d="M0 0H183.333V100H0z"></path>
      <defs>
        <pattern
          id="pattern6"
          width="1"
          height="1"
          patternContentUnits="objectBoundingBox"
        >
          <use href="#image0_268_2518" transform="scale(.00227 .00417)"></use>
        </pattern>
        <image
          id="image0_268_2518"
          width="440"
          height="240"
          href="data:image/png;base64,"
        ></image>
      </defs>
    </svg>
  );
}
